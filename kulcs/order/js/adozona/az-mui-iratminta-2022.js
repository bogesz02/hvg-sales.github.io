...
function IsOrderPossible(isItAsync, callback) {
    var data = '{ "productWebId": "AZ_MUI_IRATMINTA"}';
        ...
        success: function (msg) {
            if (!callback) {
                callback = function (ret) {
                    if (ret === false) {
                        alert("Önnek már van megrendelése a Munkaügyi szerződés- és iratmintatár 2022 csomagra. Hozzáférési problémák esetén keresse ügyfélszolgálatunkat munkanapokon 9:00-től 14:00 óráig, az ugyfelszolgalat@hvg.hu email címen vagy a (+36-1)-436 2045 telefonszámon.");
                    }
                };
            }
            ret = callback(msg.d);
...
function DoValidationextraInputData() {
    var ret = IsOrderPossible(false, function (orderIsPossible) {
        ...
        {
            $('#kupon').val('MUI2215');
            console.log('set kupon to MUI2215');
        }
